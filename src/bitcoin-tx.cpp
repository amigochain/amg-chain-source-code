...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s amigo-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  amigo-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded amigo transaction") + "\n" +
              "  amigo-tx [options] -create [commands]   " + _("Create hex-encoded amigo transaction") + "\n" +
              "\n";
...
static void MutateTxVersion(CMutableTransaction& tx, const std::string& cmdVal)
{
    ...
    if (newVersion < 1 || newVersion > CTransaction::MAX_STANDARD_AMIGO_VERSION)
        throw std::runtime_error("Invalid TX version requested");

    if (!tx.IsAmigoVersion() && IsAmigoTxVersion(newVersion))
    {
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        tx.nVersion = CTransaction::CURRENT_AMIGO_VERSION;
        int startArg;
