...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-rpccorsdomain=<domain>", _("Allow JSON-RPC connections from specified domain (e.g. http://localhost:4200 or \"*\"). This needs to be set if you are using the Amigo GUI in a browser."));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/amigo/amigo-core>";
    const std::string URL_WEBSITE = "<https://amigo.io/>";
...
bool AppInitParameterInteraction()
{
    ...
    fAmigoMode = !gArgs.GetBoolArg("-legacymode", false); // qa tests
    if (gArgs.GetBoolArg("-regtest", false) && !fAmigoMode)
        ResetParams(CBaseChainParams::REGTEST, fAmigoMode);
...
bool AppInitMain()
{
    ...
    if (fAmigoMode) // SMSG breaks functional tests with services flag, see version msg
