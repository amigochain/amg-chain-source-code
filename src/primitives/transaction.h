...
static const uint8_t AMIGO_BLOCK_VERSION = 0xA0;
static const uint8_t AMIGO_TXN_VERSION = 0xA0;
static const uint8_t MAX_AMIGO_TXN_VERSION = 0xBF;
...
inline bool IsAmigoTxVersion(int nVersion)
{
    return (nVersion & 0xFF) >= AMIGO_TXN_VERSION;
}
...
inline void SerializeTransaction(const TxType& tx, Stream& s) {
    ...
    if (IsAmigoTxVersion(tx.nVersion))
    {
...
public:
    ...
    static const int32_t CURRENT_AMIGO_VERSION=0xA0;
    ...
    static const int32_t MAX_STANDARD_AMIGO_VERSION=0xA0;
...
public:
    ...
    bool IsAmigoVersion() const {
        return IsAmigoTxVersion(nVersion);
    }
    ...
    {
        return IsAmigoTxVersion(nVersion) ? vpout.size() : vout.size();
    }
