...
extern bool fAmigoMode;


BasicTestingSetup::BasicTestingSetup(const std::string& chainName, bool fAmigoModeIn)
{
    fAmigoMode = fAmigoModeIn;
    ...
    ResetParams(chainName, fAmigoMode);
...
TestingSetup::TestingSetup(const std::string& chainName, bool fAmigoModeIn) : BasicTestingSetup(chainName, fAmigoModeIn)
{
    ...
    ClearDatadirCache();
    pathTemp = fs::temp_directory_path() / strprintf("test_amigo_%lu_%i", (unsigned long)GetTime(), (int)(InsecureRandRange(100000)));
