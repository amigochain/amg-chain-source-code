...
const char * const BITCOIN_CONF_FILENAME = "amigo.conf";
const char * const BITCOIN_PID_FILENAME = "amigod.pid";
const char * const DEFAULT_DEBUGLOGFILE = "debug.log";

bool fAmigoMode = true;
bool fAmigoWallet = false;
ArgsManager gArgs;
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Amigo
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Amigo
    // Mac: ~/Library/Application Support/Amigo
    // Unix: ~/.amigo
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Amigo";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Amigo";
#else
    // Unix
    return pathRet / ".amigo";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good())
        return; // No amigo.conf file is OK
