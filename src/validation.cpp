...
#if defined(NDEBUG)
# error "Amigo cannot be compiled without assertions."
#endif
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!fAmigoMode  // genesis coinbase is spendable when in Amigo mode
        && fIsGenesisBlock) {
        if (!fJustCheck)
    ...
    blockundo.vtxundo.reserve(block.vtx.size() - (fAmigoMode ? 0 : 1));
    ...
    } else
    {
        ...
        if (block.vtx[0]->GetValueOut() > blockReward) // amigo coins are imported as coinbase txns
...
void UpdateTip(const CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int i = 0; i < 100 && pindex != nullptr; i++)
        {
            if (fAmigoMode)
            {
                if (pindex->nVersion > AMIGO_BLOCK_VERSION)
                    ++nUpgraded;
            } else
            {
