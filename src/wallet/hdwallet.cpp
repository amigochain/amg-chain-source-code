...
std::string CHDWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageGroup(_("Amigo wallet options:"));
    ...
    unsigned int nBytes;
    {
        ...
        for (;;)
        {
            ...
            for (const auto &coin : setCoins)
            {
                ...
                if (!ProduceSignature(DummySignatureCreatorAmigo(this), scriptPubKey, sigdata))
...
bool CHDWallet::SignBlock(CBlockTemplate *pblocktemplate, int nHeight, int64_t nSearchTime)
{
    ...
    CKey key;
    pblock->nVersion = AMIGO_BLOCK_VERSION;
